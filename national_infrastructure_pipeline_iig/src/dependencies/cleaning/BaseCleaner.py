...
class BaseCleaner:
    ...
    def load_conversion_dictionaries(self, paths):
        ...
        try:
            # country_code2name = pd.read_excel(paths["country_region_incomegrp"],sheet_name='Country_income_grp')
            country_code2name = read_excel_from_buffer_bucket(
                self.bucket,
                sheet_name="Country_income_grp",
                rel_path=paths["region_and_income_group"],
            )
            self.country_code2name_dict = dict(
                country_code2name[["Code", "Country Name"]].values.tolist()
            )
        except Exception as e:
            print("Error loading dict:", e)

        try:
            # country_code2region_name = pd.read_excel(
            #   paths["country_region_incomegrp"],
            #   sheet_name='Country_income_grp'
            # )
