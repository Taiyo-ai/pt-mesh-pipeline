...
class EPIIngestion:
    def __init__(self, config) -> None:
        self.config = config
        self.index = "tenders_epihvtcentral"
    ...
    def load_data(self, path):
        try:
            ...
            df["timestamps"] = df["timestamps"].apply(lambda x: eval(x))
            df["timestamp_range"] = df["timestamp_range"].apply(lambda x: eval(x))
...
if __name__ == "__main__":
    config = load_config_yaml()["paths"]["EPIHVTC"]
